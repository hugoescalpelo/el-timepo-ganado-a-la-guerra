...
int currentVolume = 15; // Volumen inicial al 50% (0-30)

// Variables para disminuir el volumen gradualmente
bool mp3Stopping = false;
unsigned long volumeDecreaseStartTime = 0;
unsigned long lastVolumeDecreaseTime = 0;
const unsigned long volumeDecreaseInterval = 100; // Intervalo entre disminuciones de volumen (en ms)
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (length == 1 && (payload[0] == '0' || payload[0] == '1')) {
    ...
    if (msg == '1') {
      ...
      if (mp3Playing && !mp3Stopping) {
        stopMp3Playback();
      } else {
        Serial.println("MP3 no está reproduciéndose o ya se está deteniendo.");
      }
    }
  } else {
...
void startMp3Playback() {
  ...
  currentVolume = 15; // Establecer el volumen al 50%
  sendCommand(0x06, 0x00, currentVolume); // Establecer el volumen
  delay(20); // Pequeña pausa para asegurar que el comando es procesado
  ...
  mp3Playing = true;
  mp3Stopping = false; // Asegurarse de que no está en proceso de detenerse
}

// Función para detener la reproducción del MP3
void stopMp3Playback() {
  if (!mp3Stopping) {
    Serial.println("Iniciando disminución gradual del volumen...");
    mp3Stopping = true;
    volumeDecreaseStartTime = millis();
    lastVolumeDecreaseTime = millis();
  }
}
...
void setup() {
  ...
  currentVolume = 15; // Volumen inicial al 50%
  sendCommand(0x06, 0x00, currentVolume); // Volumen al 50%
  delay(20); // Pequeña pausa para asegurar que el comando es procesado
  sendCommand(0x09, 0x00, 0x02); // Seleccionar tarjeta SD
  delay(20);
...
void loop() {
  ...
  // Disminuir el volumen gradualmente al detener la reproducción
  if (mp3Stopping) {
    if (millis() - lastVolumeDecreaseTime >= volumeDecreaseInterval) {
      lastVolumeDecreaseTime = millis();
      if (currentVolume > 0) {
        currentVolume--;
        sendCommand(0x06, 0x00, currentVolume);
        ...
        mp3Playing = false;
        mp3Stopping = false;
        Serial.println("Reproducción de MP3 detenida.");
      }
    }
  }
}
